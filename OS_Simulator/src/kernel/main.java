...
public class main {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		System.out.println("Welcome to Death Megatron OS");
			...
			do {
				...
				case 1:
					...
					if(readyQueue.getSize()>0) {
						...
						try {
						status = readyQueue.queueDequeue().execute(TIME_QUANTUM, jobQueue, readyQueue, IOQueue, semaph);
					...
					status = IOQueue.queueDequeue().execute(TIME_QUANTUM, jobQueue, readyQueue, IOQueue, semaph);
					break;
				case 2:
				...
				System.out.println("Round Robin Executuon Stats:-----------\n"
						+ "Time Complemeted: "+ (RRFinish - RRStart) + "\n"
				...
				if(scan.next().charAt(0)=='y') run =  true;
				else run = false;
